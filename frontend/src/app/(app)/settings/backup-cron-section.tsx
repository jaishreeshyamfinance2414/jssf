...
export function BackupCronSection() {
  ...
  return <div className="rounded-md border p-4 space-y-3">
    ...
    {loading ? <p className="text-sm text-muted-foreground">Checking server cron…</p> : <>
      {status?.available ? <>
        <p className="text-xs">Current job: <strong>{status.enabled ? 'Enabled' : 'Disabled'}</strong>{status.enabled ? ` at ${status.time} ${status.timezone ?? ''}` : ''}</p>
        ...
        <div><p className="text-xs font-medium">Installed script path</p><code className="break-all text-xs text-muted-foreground">{status.scriptPath}</code></div>
        <div className="rounded-md bg-muted/50 p-3 text-sm space-y-1">
          ...
          {status.lastRun?.state === 'failed' && <><p className="text-danger">Failed {status.lastRun.finishedAt ? new Date(status.lastRun.finishedAt).toLocaleString() : ''}.</p><pre className="whitespace-pre-wrap break-words text-xs text-danger">{status.lastRun.error ?? 'The backup script did not report a specific error. Check /var/log/jssf-backup.log on the server.'}</pre></>}
          ...
          {(!status.lastRun || status.lastRun.state === 'never') && <p className="text-muted-foreground">No backup run has been recorded yet.</p>}
        </div>
        <Button size="sm" onClick={() => void save()} disabled={saving || !time}>{saving ? 'Saving…' : 'Save cron job'}</Button>
      </> : <p className="text-sm text-danger">{status?.message ?? 'Cron setup is unavailable on this server.'}</p>}
