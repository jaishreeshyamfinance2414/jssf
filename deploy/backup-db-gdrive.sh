#!/usr/bin/env bash
# ============================================================
# ARCHIVED — Original backup script with Google Drive + rclone support
#
# This file is an unmodified copy of the original backup-db.sh that
# supported three off-server destinations:
#   1. AWS S3            (aws CLI)
#   2. Google Drive      (rclone — requires OAuth, eventually became paid)
#   3. Backblaze B2      (aws CLI, S3-compatible API)
#
# It also used rclone to copy customer documents from Cloudflare R2 to
# Google Drive as a secondary backup.
#
# WHY THIS FILE EXISTS:
#   Google Drive OAuth tokens require periodic re-authorization and the
#   Google API eventually moved to a paid tier. We switched to a simpler
#   setup using only AWS CLI:
#     - Backblaze B2 for database backups (application data)
#     - Cloudflare R2 for customer documents (already the live store)
#
#   If Google Drive backup is ever needed again, this file can be
#   restored as backup-db.sh. See BACKUP-LEGACY.md for details.
#
# DATE ARCHIVED: 2026-09-12
# REPLACED BY:   backup-db.sh (AWS CLI only, no rclone)
# ============================================================
